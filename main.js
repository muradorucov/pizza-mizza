...
searchElem.addEventListener("keyup", (event) => {
    if (event.key === "Enter") {
        ...
        const filteredList = data.filter(pizza => {
            return pizza.name.toLowerCase().includes(val) ||
                pizza.dsc.toLowerCase().includes(val)
        })
        start = 0;
        end = 12;
...
function domRender(list) {
    ...
    list.slice(start, end).forEach(pizza => {
        pizzasElem.innerHTML += `
...
function generatePagination(param) {
    let totalPages = Math.ceil(param.length / 12);
    paginationsElem.innerHTML = ""
    for (let i = 1; i <= totalPages; i++) {
        paginationsElem.innerHTML +=
            `
         <button class="bg-[blue] text-[#fff] shadow-lg w-[50px] h-[40px] flex justify-center items-center cursor-pointer"
         onclick="getPagination(${i})"
         >
               ${i}
                </button>
        `
...
function getPagination(num = 1) {
    start = (num - 1) * 12;
    end = num * 12;
    domRender(globalList)
    // domRender()
}
